...
/// \param category 라벨 카테고리
///
/// \param count 최대 개수.
///
- (void)setMaxCountPerTileWithCategory:(LabelCategory)category count:(NSUInteger)count;
/// 타일별 최대 개수 지정값을 초기값으로 되돌린다.
/// \param category 라벨 카테고리
///
- (void)resetMaxCountPerTileWithCategory:(LabelCategory)category;
/// 특정 카테고리의 지도 기본 라벨의 표시 유무를 지정한다.
/// \param category 라벨 카테고리
///
/// \param visible 표시여부.
///
- (void)setLabelCategoryVisibleWithCategory:(LabelCategory)category visible:(BOOL)visible;
...
///
/// returns:
/// 이 KakaoMap의 ZoneManager
- (ZoneManager * _Nonnull)getZoneManager SWIFT_WARN_UNUSED_RESULT;
/// 이벤트 delegate를 지정한다.
...
/// \param kakaoMap KakaoMap
///
/// \param zoneType Zone Type (ex. indoor)
///
/// \param zoneId CheckRect 안에 들어온 zone의 Id
///
/// \param zoneDetailId zone의 기본 detail Id
///
/// \param zoneDetails zone의 detail Id의 목록
///
/// \param zoneLinkInfos zone 연결정보 dictionary. key : 현재 zoneDetail 중에서 다른 zone과 연결된 부분이 있는 detailId. value : 연결된 다른 zone-detail 리스트.
///
- (void)onEnterZoneWithKakaoMap:(KakaoMap * _Nonnull)kakaoMap zoneType:(NSString * _Nonnull)zoneType zoneId:(NSString * _Nonnull)zoneId zoneDetailId:(NSString * _Nonnull)zoneDetailId zoneDetails:(NSArray<NSString *> * _Nonnull)zoneDetails zoneLinkInfos:(NSDictionary<NSString *, NSArray<ZoneLinkInfo *> *> * _Nonnull)zoneLinkInfos;
/// Zone을 나갔을 때(zone이 checkRect 밖으로 나갔을 때) 호출
/// \param kakaoMap KakaoMap
///
/// \param zoneType Zone Type (ex. indoor)
///
/// \param zoneId CheckRect 밖으로 나간 zone의 Id
///
/// \param zoneDetailId zone의 기본 detail Id
///
/// \param zoneDetails zone의 detail Id의 목록
///
- (void)onLeaveZoneWithKakaoMap:(KakaoMap * _Nonnull)kakaoMap zoneType:(NSString * _Nonnull)zoneType zoneId:(NSString * _Nonnull)zoneId zoneDetailId:(NSString * _Nonnull)zoneDetailId zoneDetails:(NSArray<NSString *> * _Nonnull)zoneDetails;
@end
...
/// MapPolygon 내부의 겹치는 hole들을 병합하여 렌더링할지 여부.
/// 기본값은 false이다. true로 설정하면 겹치는 hole 영역을 하나로 합쳐서 표시한다.
@property (nonatomic) BOOL mergeOverlappedHoles;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// 마커 심볼 이미지
@property (nonatomic, strong) UIImage * _Nullable symbol;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// PolygonStyleSet에서 사용할 표출 스타일 인덱스를 지정한다.
@property (nonatomic, readonly) NSUInteger styleIndex;
/// 폴리곤 내부의 겹치는 hole들을 병합하여 렌더링할지 여부.
/// 기본값은 false이다. true로 설정하면 겹치는 hole 영역을 하나로 합쳐서 표시한다.
@property (nonatomic) BOOL mergeOverlappedHoles;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
...
SWIFT_PROTOCOL("_TtP12KakaoMapsSDK20RouteAnimationEffect_")
@protocol RouteAnimationEffect
/// 애니메이션 효과 옵션
@property (nonatomic) struct AnimationInterpolation interpolation;
...
SWIFT_CLASS("_TtC12KakaoMapsSDK23ProgressAnimationEffect")
@interface ProgressAnimationEffect : NSObject <RouteAnimationEffect>
/// Initializer
/// \param direction progress 진행방향
///
/// \param type RouteLine의 진행타입.
///
- (nonnull instancetype)initWithDirection:(ProgressDirection)direction type:(RouteProgressType)type OBJC_DESIGNATED_INITIALIZER;
/// 시작지점(0.0~1.0)
@property (nonatomic, readonly) float startPoint;
/// 종료지점(0.0~1.0)
@property (nonatomic, readonly) float endPoint;
/// progress 진행 방향
@property (nonatomic) ProgressDirection direction;
/// RouteLine 진행 타입
@property (nonatomic) RouteProgressType type;
/// 애니메이션 지속 시간, 반복 횟수 등 세부사항 지정
@property (nonatomic) struct AnimationInterpolation interpolation;
/// 애니메이션 종료 시 애니메이터에 속한 객체들을 숨길지에 대한 여부.
/// true로 설정하면 애니메이션이 종료되면 객체들이 화면에서 자동으로 사라진다.
...
///
- (void)roadviewResultDidReceivedWithRoadview:(Roadview * _Nonnull)roadview panoID:(NSString * _Nonnull)panoID date:(NSString * _Nonnull)date position:(MapPoint * _Nonnull)position;
@optional
...
/// \param viewName view의 이름
///
/// \param appName Cocoa에 등록된 app 이름. 따로 등록된 내용이 없을 경우 “roadview” 사용.
///
/// \param viewInfoName viewInfo의 이름. 미지정시 type에 따른 기본값.
///
/// \param enabled 초기 활성화 여부. 기본값은 true.
///
- (nonnull instancetype)initWithViewName:(NSString * _Nonnull)viewName appName:(NSString * _Nonnull)appName viewInfoName:(NSString * _Nonnull)viewInfoName enabled:(BOOL)enabled;
@end
...
/// \param position 경로선 상의 진행도를 얻어올 위치
///
///
/// returns:
/// 위치에 해당하는 진행도 (0.0 ~ 1.0).
- (float)getProgressAlongRouteLineWithPosition:(MapPoint * _Nonnull)position SWIFT_WARN_UNUSED_RESULT;
...
///   </li>
/// </ul>
- (void)setProgressWithProgress:(float)progress type:(RouteProgressType)type duration:(NSUInteger)duration callback:(void (^ _Nullable)(Route * _Nullable))callback;
/// Route의 ID
...
SWIFT_CLASS("_TtC12KakaoMapsSDK13RouteAnimator")
@interface RouteAnimator : NSObject <Animator>
/// Animator를 동작시킨다.
...
/// \param poi Animator에 추가할 route
///
- (void)addRoute:(Route * _Nonnull)route;
/// Animator에 여러개의 route를 추가한다.
/// 등록한 Animation에 동작시키고자 하는 route를 추가한다. start() 를 호출한 이후에는 애니메이션이 끝나기 전까지 route를 추가할 수 없다.
/// \param pois Animator에 추가할 route 배열
///
- (void)addRoutes:(NSArray<Route *> * _Nonnull)routes;
...
/// returns:
/// 생성된 RouteAnimator
- (RouteAnimator * _Nullable)addRouteAnimatorWithAnimatorID:(NSString * _Nonnull)animatorID effect:(id <RouteAnimationEffect> _Nonnull)effect SWIFT_WARN_UNUSED_RESULT;
/// 추가한 RouteAnimator를 삭제한다.
/// \param animatorID 삭제할 animator ID
///
- (void)removeRouteAnimatorWithAnimatorID:(NSString * _Nonnull)animatorID;
/// 추가한 모든 RouteAnimator를 제거한다.
- (void)clearAllRouteAnimators;
/// 추가한 Animator를 가져온다.
/// \param animatorID animatorID
///
///
/// returns:
/// RouteAnimator
- (RouteAnimator * _Nullable)getRouteAnimatorWithAnimatorID:(NSString * _Nonnull)animatorID SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
///   </li>
/// </ul>
+ (void)InitSDKWithAppKey:(NSString * _Nonnull)appKey phase:(KAPhase)phase;
+ (KAPhase)GetPhase SWIFT_WARN_UNUSED_RESULT;
/// AppKey
///
/// returns:
/// appKey
...
SWIFT_CLASS("_TtC12KakaoMapsSDK11ZoneManager")
...
///   </li>
/// </ul>
- (void)setZoneCheckRectScaleWithZoneType:(NSString * _Nonnull)zoneType level:(NSInteger)level scale:(struct Vector2)scale;
/// Zone의 상세 레이어를 표시한다.
...
///   </li>
/// </ul>
- (void)showZoneDetailWithZoneType:(NSString * _Nonnull)zoneType zoneId:(NSString * _Nonnull)zoneId detailId:(NSString * _Nonnull)detailId;
/// Zone의 상세 레이어를 숨긴다.
/// \param zoneType 상세 레이어를 숨길 zone의 type
///
- (void)hideZoneDetailWithZoneType:(NSString * _Nonnull)zoneType;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
